...
class EvrinomaAdvantageExtension extends Extension
{
    use HelperTrait;

    public const ENTITY = 'Evrinoma\AdvantageBundle\Entity';
    public const MODEL = 'Evrinoma\AdvantageBundle\Model';
    public const ENTITY_FACTORY_ADVANTAGE = AdvantageFactory::class;
    public const ENTITY_BASE_ADVANTAGE = BaseAdvantage::class;
    public const DTO_BASE_ADVANTAGE = AdvantageApiDto::class;
    ...
    public function load(array $configs, ContainerBuilder $container)
    {
        ...
        if (self::ENTITY_FACTORY_ADVANTAGE !== $config['factory']) {
    ...
    private function wireConstraintTag(ContainerBuilder $container): void
    {
        foreach ($container->getDefinitions() as $key => $definition) {
            switch (true) {
                case str_contains($key, AdvantagePass::ADVANTAGE_CONSTRAINT)   :
                    $definition->addTag(AdvantagePass::ADVANTAGE_CONSTRAINT);
                    break;
                default:
            }
        }
    ...
    private function wireRepository(ContainerBuilder $container, Reference $registry, string $madiator, string $class, string $driver): void
    {
        ...
        $container->addAliases([AdvantageCommandRepositoryInterface::class => 'evrinoma.'.$this->getAlias().'.repository']);
        $container->addAliases([AdvantageQueryRepositoryInterface::class => 'evrinoma.'.$this->getAlias().'.repository']);
    }
    ...
    {
        return EvrinomaAdvantageBundle::BUNDLE;
    }
}
